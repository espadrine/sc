...
function getQueries(ask, end) {
  ...
  if (ask.req.method === 'GET' || ask.req.method === 'HEAD') {
    ...
    if (multipart.test(contentType)) {
      // Multipart data.
      ask.form = ask.form || new formidable.IncomingForm();
...
function getCompressedStream(ask) {
  var encoding = ask.req.headers['accept-encoding'] || '';
  var res = ask.res;
...
function Camp(opts) {
  ...
  this.documentRoot = opts.documentRoot || p.join(process.cwd(), 'web');
  this.stack = [];
...
function genericUnit (server) {
  ...
  server.removeHandler = function(f) {
  ...
  return function genericLayer (ask, next) {
    for (var i = 0; i < processors.length; i++) {
      var keep = processors[i](ask);
...
function staticUnit (server) {
  ...
  return function staticLayer (ask, next) {
    ...
    fs.stat(realpath, function(err, stats) {
      ...
      var res = getCompressedStream(ask);
...
function routeUnit (server) {
  ...
  return function routeLayer (ask, next) {
    ...
    for (var i = 0; i < regexes.length; i++) {
      matched = ask.path.match (regexes[i]);
      if (matched !== null) { cbindex = i; break; }
    }
    if (cbindex >= 0) {
      catchpath(ask, matched, callbacks[cbindex], server.templateReader);
    } else {
      next();
    }
  };
...
function catchpath (ask, pathmatch, callback, templateReader) {
  var res = ask.res;
  ...
  getQueries(ask, function gotQueries(err) {
    if (err !== null) {
      ...
      callback(ask.query, pathmatch, function end (params, options) {
        options = options || {};
        ...
        if (!ask.res.getHeader('Content-Type')) {  // Allow overriding.
          ask.mime(mime[p.extname(templates[0]).slice(1)] || 'text/plain');
        }
        for (var i = 0; i < templates.length; i++) {
          if (typeof templates[i] === 'string') {
            // `templates[i]` is a string path for a file.
            var templatePath = p.join(ask.server.documentRoot, templates[i]);
