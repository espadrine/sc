...
function augmentReqRes(req, res, server) {
  ...
  res.json = function (data) {
    ...
    // This gets compressed anyway, so we might as well make it human-readable
    var json = JSON.stringify(data, null, 2) + '\n';
    getCompressedStream(req, res).end(json);
  };
}
...
var KEY_HEADER = '-----BEGIN RSA PRIVATE KEY-----\n';
...
function startServer (settings) {
  ...
  if (settings.secure) { // Yep
    var key, cert, ca;
    if (settings.key.slice(0, KEY_HEADER.length) === KEY_HEADER) {
      key = settings.key;
    } else {
      key = fs.readFileSync(settings.key);
    }
    if (settings.cert.slice(0, CERT_HEADER.length) === CERT_HEADER) {
      cert = settings.cert;
    } else {
