...
function augmentReqRes(req, res, server) {
  ...
  res.template = function(scope, templates) {
    if (templates.length > 0) {
      res.mime(p.extname(templates[0].path).slice(1));
    }
...
// path: path to templating file (String).
// options:
...
function template(path, options) {
  options = options || {};
  path = protectedPath(this.documentRoot, p.join(this.documentRoot, path));
...
var defaultRoute = [genericUnit, wsUnit, ajaxUnit, eventSourceUnit,
                    pathUnit, routeUnit, staticUnit, notfoundUnit];
...
function Path(matcher) {
  ...
  if (matcher instanceof RegExp) {
    ...
    if (matcher.indexOf(':') >= 0 || matcher.indexOf('*') >= 0) {
      console.warn('ScoutCamp does not currently support parameters.');
    }
    this.match = function(path) {
      if (path === matcher) { return true;
      } else { return null; }
    };
  }
}
    ...
    if (cbindex >= 0) {
      getQueries(req, function(err) {
        ...
        if (callbacks[cbindex] !== undefined) {
          ...
          // We remove the initial slash from the path.
          res.template(req.data, [server.template(req.path.slice(1))]);
        }
      });
    } else { next(); }
  };
