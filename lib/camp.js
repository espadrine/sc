...
platehandled = {};
exports.format = function (path, literalcall) {
  platehandled[path] = literalcall;
};
    ...
    try {
      ...
      if (/^\/\$/.test (path)) {
          ...
          if (exports.Server.Actions[action]) {
            ...
            if (typeof sentback === 'object') {
              ...
              exports.Server.on (listen, function listencb () {
                ...
                if (debug) { console.log ('event',listen,
        ...
        fs.readFile(realpath
            ...
            , function (err, data) {
          if (err) {
            if (debug) { console.log ('File not found:', err.path); }
            ...
            if (platehandled[path] !== undefined) {
              res.write (Plate.format (data.toString ()
                  , platehandled[path] (query)));
            } else {
