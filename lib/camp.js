...
function ajaxUnit (server) {
  ...
  return function ajaxLayer (ask, next) {
    ...
    parseQuery(ask, function(err) {
      if (err == null) {
        ajax.emit(action, ask.query, function ajaxEnd(data) {
          res.end(JSON.stringify(data || {}));
        }, ask);
      } else {
        log('While parsing', ask.req.url + ':\n'
          + err
          , 'error');
        return next();
      }
...
// Use a compressed stream if possible.
function compressed (ask) {
...
function staticUnit (server) {
  ...
  return function staticLayer (ask, next) {
    ...
    var realpath = protectedPath(documentRoot, p.join(documentRoot, ask.path));
    fs.stat(realpath, function(err, stats) {
      ...
      raw.pipe(compressed(ask));
    });
  };
}

// Generic routing unit.
function routingEngine (server, name) {
  var routes = [];

  server[name] = function route (pattern, handler) {
    routes.push({
      pattern: RegExp(pattern).source,
      handler: handler
    });
  };

  return function handle (ask, next) {
    var filteredRoutes = routes.filter(function (route) {
      return RegExp(route.pattern).test(ask.path);
    });
    ...
    if (filteredRoutes.length > 1) {
      var warnMsg = 'More than one template path matches for ' + ask.path + ':\n';
      filteredRoutes.forEach (function (route) { warnMsg += '- ' + route.pattern; });
      log(warnMsg, 'warn');
    }
    var match = ask.path.match (RegExp (filteredRoutes[0].pattern));

    parseQuery(ask, function gotQuery (err) {
      if (err !== null) {
        log('While getting queries for ' + ask.uri + ':\n'
          + err
          , 'error');
      } else {
        // params: template parameters (JSON-serializable).
        filteredRoutes[0].handler(ask.query, match, function end (params, options) {
          options = options || {};
          options.template = options.template || match[0];
          options.reader = options.reader || server.templateReader;
          ...
          if (!(params && Object.keys(params).length)) {
            ...
            reader.pipe(compressed(ask));
          } else {
            options.reader(reader, compressed(ask), params, function errorcb(err) {
              if (err) {
                log(err.stack, 'error');
                ask.res.end('404\n');
              }
...
// Template routing unit.
function routeUnit (server) {
  var router = routingEngine(server, 'route');
...
function notfoundUnit (server) {
  var router = routingEngine(server, 'notfound');

  return function handleNotfound (ask) {
    ask.res.statusCode = 404;
    router(ask, function next () {
      ask.res.end('404\n');
    });
  }
}
