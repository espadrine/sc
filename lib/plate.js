...
Plate.fmt = function (text, literal) {
  var opencurl = /(?:^|[^\{])\{\{[^\{]/;
  var closecurl = /(?:^|[^\}])\}\}(?!\})/;
  ...
  return Plate._escape (text.slice (0, operation.index)) +
      ...
      Plate.fmt (text.slice (nextcurl), literal));
...
Plate.macros = {
  '=': function (literal, params) {
    ...
    if (literal[params[0]] === undefined) {
      console.log ('Plate error: literal ' + JSON.stringify (params[0]) +
                   ' is missing.');
      return '';
    }
    return Plate.parsers[params[1]] (literal[params[0]], params[2]);
  },
  '?': function (literal, params) {
    if (literal[params[0]]) {
      return params[1];
  ...
  '-': function (literal, params) {
    if (literal[params[2]] === undefined) {
      console.log ('Plate error: literal ' + JSON.stringify (params[2]) +
                   ' is missing.');
      return '';
    }
    var list = '';
    ...
    for (var i in literal[params[2]]) {
      ...
      list += Plate.fmt (params[3], literal);
    }
    return list;
  },
  '#': function () { return ''; },
...
Plate.parsers = {
  ...
  'intradix': function (intradix, radix) {
    return intradix.toString (parseInt (radix));
  },
  'float': function (floating, fractionDigits) {
    return floating.toFixed (parseInt (fractionDigits));
  },
  'exp': function (exp, fractionDigits) {
    return exp.toExponential (parseInt (fractionDigits));
  }
};
